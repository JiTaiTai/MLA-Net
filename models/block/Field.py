...
class ASPP3(nn.Module):
    def __init__(self, in_channels, atrous_rates=(2,3,5,7)):
        ...
        self.b3 = nn.Sequential(
            ...
            nn.ReLU(True))
        self.b4 = nn.Sequential(
            nn.Conv2d(out_channels, out_channels, (3, 3), padding=rate4, dilation=rate4, bias=False),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(True))
        ...
        self.dim_reduction = Conv3Relu(out_channels * 6, in_channels)
        self.dim_reduction2 = Conv3Relu(out_channels * 2, in_channels)
    ...
    def forward(self, x):
        ...
        out1 = self.dim_reduction(torch.cat((x,feat1, feat2, feat3, feat4), 1))
        #out = self.dim_reduction2(torch.cat((add4, out1), 1))
