...
class CSWinTransformer(nn.Module):
    ...
    """
    def __init__(self, img_size=256, patch_size=16, in_chans=3, num_classes=1000, embed_dim=96, depth=[2,2,6,2], split_size = [3,5,7],
...
def CSWin_64_12211_tiny_224(pretrained=False, **kwargs):
    model = CSWinTransformer(img_size=256, patch_size=4, embed_dim=64, depth=[1, 2, 21, 1],
    ...
    if pretrained:
        pretrained_file = "pretrain/cswin/cswin_tiny_224.pth"
...
def CSWin_64_24322_small_224(pretrained=False, **kwargs):
    ...
    if pretrained:
        pretrained_file = "pretrain/cswin/cswin_small_224.pth"
        pretrained_dict = torch.load(pretrained_file)["state_dict_ema"]
        model_dict = model.state_dict()
        pretrained_dict = {k: v for k, v in pretrained_dict.items()
                           if k in model_dict.keys()}
...
def CSWin_96_24322_base_224(pretrained=False, **kwargs):
    ...
    if pretrained:
        pretrained_file = "pretrain/cswin/cswin_base_224.pth"
        pretrained_dict = torch.load(pretrained_file)["state_dict_ema"]
        model_dict = model.state_dict()
        pretrained_dict = {k: v for k, v in pretrained_dict.items()
                           if k in model_dict.keys()}
...
def CSWin_96_24322_base_384(pretrained=False, **kwargs):
    ...
    if pretrained:
        pretrained_file = "pretrain/cswin/cswin_base_384.pth"
        pretrained_dict = torch.load(pretrained_file)["state_dict_ema"]
        model_dict = model.state_dict()
        pretrained_dict = {k: v for k, v in pretrained_dict.items()
                           if k in model_dict.keys()}
